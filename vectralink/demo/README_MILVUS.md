...
- Milvus (Testcontainers) persists vectors and performs similarity search.
- A Vaadin map UI lets you insert random cities and click a city to find nearby ones by vector similarity.

Packages live under `ai.cyrock.vectralink.demo-milvus` subpackages.
...
- Configuration:
  - `config/ContainerConfig.java` launches MySQL 5.7.34 and Milvus 2.6.6 (+ an additional etcd container) via Testcontainers and exposes them on localhost (3306, 9091/19530, 2379).
  - `config/VectralinkConfig.java` wires the Vectralink pipeline:
    ...
    - `MilvusVectorStore` (collection `test`, size 3, cosine distance).
...
- Milvus README: `../milvus/README.md`
...
- Docker Desktop (or compatible) running — required by Testcontainers
  - Free local ports: 3306 (MySQL), 9091/19530 (Milvus) + 2379 (etcd), 8080 (demo UI)
...
- `VectorStoreUpdater` uses:
  ...
  - The `MilvusVectorStore` to insert/delete vectors accordingly.
...
// Milvus store: size=3, cosine distance
@Bean
MilvusVectorStore milvusVectorStore(final MilvusContainer milvus) throws Exception { /* ... */ }

// Debezium -> VectorStoreUpdater pipeline
@Bean(destroyMethod = "stop")
DebeziumConnector getDebeziumConnector(Vectorizer<Vector.Float> vectorizer, MilvusVectorStore vectorStore) { /* ... */ }
...
- Milvus collection `example_collection_milvus` is created at startup with required fields, vector size 3 and cosine distance.
...
- Milvus vector store: `../milvus/README.md`
