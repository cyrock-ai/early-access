...
- Weaviate (Testcontainers) persists vectors and performs similarity search.
- A Vaadin map UI lets you insert random cities and click a city to find nearby ones by vector similarity.

Packages live under `ai.cyrock.vectralink.demo-weaviate` subpackages.

Tip: This Demo app runs under port **`8081`** instead of `8080`
...
- Configuration:
  - `config/ContainerConfig.java` launches MySQL 5.7.34 and Weaviate 1.34.4 via Testcontainers and exposes them on localhost (8080, 50051).
  - `config/VectralinkConfig.java` wires the Vectralink pipeline:
    ...
    - `WeaviateVectorStore` (collection `Demo_collection_weaviate`, cosine distance).
...
- Docker Desktop (or compatible) running — required by Testcontainers
  - Free local ports: 3306 (MySQL), 8080/50051 (Weaviate), 8081 (demo UI)
...
- `VectorStoreUpdater` uses:
  ...
  - The `WeaviateVectorStore` to insert/delete vectors accordingly.
...
// Weaviate store: size=3, cosine distance
@Bean
WeaviateVectorStore weaviateVectorStore(WeaviateContainer weaviate) throws Exception { /* ... */ }

// Debezium -> VectorStoreUpdater pipeline
@Bean(destroyMethod = "stop")
DebeziumConnector getDebeziumConnector(Vectorizer<Vector.Float> vectorizer, WeaviateVectorStore vectorStore) { /* ... */ }
...
- Weaviate collection `Demo_collection_weaviate` is created at startup with required fields and cosine distance.
...
- Weaviate vector store: `../weaviate/README.md`
